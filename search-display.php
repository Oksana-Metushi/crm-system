...
<section class="pt-12 md:pt-20 pb-6 px-4 max-w-screen-xl mx-auto" style="font-family:'Lato',sans-serif;">
    <div class="pt-10 mx-auto flex justify-between mb-6">
        <div class="text-4xl font-bold text-white text-center">Search</div>
    </div>
    <?php
    if (isset($_POST['submit'])) {
        $search = $_POST['search'];
        $sql = "SELECT * FROM customers WHERE id LIKE '%$search%'  OR customername LIKE '%$search%'";
        $result = mysqli_query($conn, $sql);
        if ($result) {
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    echo "
         <div class='overflow-hidden rounded-lg border bg-white border-gray-200 shadow-md mx-auto p-2'>
        ...
        <tr class='hover:bg-gray-50'>
                    ...
                    <td class='px-6 py-4'>
                        <div class='flex justify-end gap-4'>
                            ...
                            <a x-data='{ tooltip: 'View' }' href='viewData.php?id=$row[id]'>View
                                </a>
                            <a x-data='{ tooltip: 'Delete' }' href='includes/delete.php?id=$row[id]'>
                                <svg fill='none' viewBox='0 0 24 24' stroke-width='1.5' stroke='currentColor' class='h-6 w-6' x-tooltip='tooltip'>
                                    <path stroke-linecap='round' stroke-linejoin='round' d='M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0' />
                                </svg>
                            </a>
                            <a x-data='{ tooltip: 'Edite' }' href='update.php?updateid=$row[id]'>
                                <svg fill='none' viewBox='0 0 24 24' stroke-width='1.5' stroke='currentColor' class='h-6 w-6' x-tooltip='tooltip'>
